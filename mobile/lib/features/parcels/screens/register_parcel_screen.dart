...
class _RegisterParcelScreenState extends State<RegisterParcelScreen> {
  ...
  Future<void> _save({bool skip = false}) async {
    // Read everything from context BEFORE any await — watch() is illegal in async
    final isUrdu      = context.read<LanguageProvider>().isUrdu;
    final parcelProv  = context.read<ParcelProvider>();
    ...
    final p = await parcelProv.createParcel(
      name:        parcelName,
      location:    location,
    ...
    if (p != null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(isUrdu ? 'پلاٹ رجسٹر ہو گیا!' : 'Parcel registered!'),
        backgroundColor: AppTheme.primary,
      ));
      context.pop();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(parcelProv.error ?? 'Error'),
              ...
              SizedBox.expand(
                child: FlutterMap(
                  ...
                  children: [
                    ...
                    // ESRI label overlay — roads, villages, city names
                    TileLayer(
...
//     setState(() => _searching = true);
//     try {
//       // Use both the query and "Pakistan" to narrow results
//       final encodedQuery = Uri.encodeComponent('$q Pakistan');
//       final uri = Uri.parse(
//           'https://nominatim.openstreetmap.org/search'
//               '?q=$encodedQuery&format=json&limit=5&addressdetails=1&countrycodes=pk');
//
//       final res = await http.get(uri, headers: {
//         // Nominatim policy requires a valid User-Agent with contact info
//         'User-Agent': 'Cropsify/1.0 (hk5028203@gmail.com)',
...
//             'Search error (${res.statusCode}). Try again.',
//             'تلاش میں خرابی (${res.statusCode})۔ دوبارہ کوشش کریں۔',
//           )),
...
//               'مقام نہیں ملا۔ قریبی شہر کا نام آزمائیں۔',
//             )),
//           ));
//         }
//       }
...
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//           content: Text(context.tr(
//             'Search timed out. Check your internet connection.',
//             'تلاش کا وقت ختم۔ انٹرنیٹ چیک کریں۔',
//           )),
//           backgroundColor: Colors.orange.shade700,
//         ));
//       }
//     } on FormatException {
//       // JSON parse failed — Nominatim returned HTML (rate limited or blocked)
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//           content: Text(context.tr(
//             'Search unavailable. Try a city chip below.',
//             'تلاش دستیاب نہیں۔ نیچے شہر کا بٹن استعمال کریں۔',
//           )),
//           backgroundColor: Colors.orange.shade700,
//         ));
//       }
...
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//           content: Text(context.tr(
//             'Search failed. Check internet and try again.',
//             'تلاش ناکام۔ انٹرنیٹ چیک کریں اور دوبارہ کوشش کریں۔',
//           )),
//           backgroundColor: Colors.red.shade700,
//         ));
//       }
...
//         Expanded(
//           child: Stack(
//             fit: StackFit.expand, // forces FlutterMap to fill — avoids layout crash
//             children: [
//
//               FlutterMap(
//                 mapController: _mapCtrl,
...
//                 ),
//                 children: [
//                   // Satellite base layer (ESRI — free, no key needed)
//                   TileLayer(
//                     urlTemplate: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
//                     userAgentPackageName: 'com.cropsify.app',
...
//                     },
//                   ),
//                   // Label overlay — roads, villages, city names
//                   TileLayer(
//                     urlTemplate: 'https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}',
//                     userAgentPackageName: 'com.cropsify.app',
//                     errorTileCallback: (tile, error, stackTrace) {
//                       debugPrint('[map] label tile error: $error');
//                     },
//                   ),
//                   if (polylines.isNotEmpty)
//                     PolylineLayer(polylines: polylines),
